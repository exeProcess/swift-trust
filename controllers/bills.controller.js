...
exports.getVendingProducts = async (req, res) => {
  const { categoryCode, provider } = req.query;

  if (!categoryCode || !provider) {
    return res.status(400).json({
      error: 'Both "categoryCode" and "provider" query parameters are required.',
    });
  }

  try {
    const data = await fetchVendingProducts(categoryCode, provider);
    res.status(200).json(data);
  } catch (error) {
    console.error('Controller error:', error.message);
    res.status(500).json({ error: 'Failed to fetch vending products from provider.' });
  }
};
